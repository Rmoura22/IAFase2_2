...
(defun alfa-beta (node profundidade-max jogador-max timestamps &optional (alfa *minimum-value*) (beta *maximum-value*))
  (cond
    ((or (eq (fifth node) profundidade-max)
         (null node) 
         (null (convert-all-to-alfa-beta-nodes (successors (convert-to-interact-node node) (get-operators))))
         (has-passed-90-percent (first timestamps) (second timestamps)))             
                            (heuristic-minmax node jogador-max))  ; 
    (t (let ((sucessores (order-nodes (convert-all-to-alfa-beta-nodes (successors (convert-to-interact-node node) (get-operators))) (sixth node) jogador-max)))
                         (cond
                           ;Nó Max
...
(defun no-teste5 ()
  (list 45 0 '(0 0) (tabuleiro-teste2) 0 *jogador1* nil nil '(9 5))
)