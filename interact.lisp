...
(defun play (node whiteFunc blackFunc &key (turn 'white) (turnNum 0) (timeLimit 3000) (whiteName "white") (blackName "black"))
  ...
  (cond
    ((equal turnNum 3) nil)
    ((game-over node) (get-winner node turn whiteName blackName))
    ((equal turn 'white)
...
(defun player-turn (node &optional (timeLimit))
  ...
  (cond 
    ...
    (t 
      (let ((movement (Read-Player-Move node (get-player-pos (get-Node-CurPlayer node)))))
        (move node (first movement) (second movement) (get-operator (first movement) (second movement)))
      )
    )
  )
)
...
(defun game-over (node)
  (and (not (can-move node)) (not (can-move (swap-players node))))
)
